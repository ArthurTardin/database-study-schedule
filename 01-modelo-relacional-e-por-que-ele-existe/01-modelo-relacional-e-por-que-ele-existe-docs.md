# Etapa 1 - Modelo Relacional e Fundamentos (SQL)

## Antes de começar

Esta é a primeira etapa do roadmap — não há conteúdo anterior pra conectar ainda. O risco novo aqui não é de sintaxe (você praticamente não escreve SQL nesta etapa), é de **ambiente**: instalar Docker no Windows, subir um container, conectar via `psql` e confirmar que o que você está vendo é real, não assumido. Erro comum nesse ponto: achar que "abriu o app" ou "vi a versão na interface" é prova de que o ambiente está funcional — não é. Prova é output de terminal.
