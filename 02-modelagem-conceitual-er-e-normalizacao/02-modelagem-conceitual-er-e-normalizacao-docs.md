# Etapa 2 - Modelagem Conceitual: ER e Normalização (1NF-3NF) (SQL)

## Antes de começar

Você já fechou a Etapa 1 com o ambiente funcional e as duas perguntas de verificação respondidas — mas vale registrar o padrão que apareceu lá: na primeira tentativa da questão sobre anomalia de atualização, você misturou esse conceito com o de integridade referencial (registro "solto" sem ligação). Essa etapa vai testar exatamente esse tipo de reflexo de novo, porque **2NF e 3NF são fáceis de confundir entre si** se você não prestar atenção em qual dependência cada uma resolve. Não adianta "saber que existe 1NF, 2NF, 3NF" — se você não conseguir dizer qual anomalia cada forma resolve especificamente, você vai cometer o mesmo tipo de erro de novo, só que num conceito diferente.
